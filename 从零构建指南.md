...
步骤 2：插入 key=13, value="B"
  hash(13) = (unsigned)13 * 2654435761u % 8
           = 34507664893u % 8
           = 5       （余数还是 5 -- 冲突！）
  slot[5] 已占用 → 线性探测 slot[6] → 空 → 放入
  表：[空][空][空][空][空][5→"A"][13→"B"][空]
                          ↑       ↑
                       原本位置  探测后位置

步骤 3：删除 key=5（惰性删除）
  找到 slot[5]，标记 deleted=true
  表：[空][空][空][空][空][(删)5→"A"][13→"B"][空]
  cnt 从 2 减到 1（有效元素数）

步骤 4：查找 key=13
  hash(13) = 5
  slot[5].used=true 但 slot[5].deleted=true → ★ 跳过，但不停止 ★
  slot[6].used=true, slot[6].deleted=false, slot[6].key==13 → 找到！
  ✓ 返回 slot[6].value = "B"

如果步骤 3 用的是"真正删除"（清空 slot）而不是惰性删除：
  表：[空][空][空][空][空][空][13→"B"][空]
  步骤 4：hash(13)=5 → slot[5].used=false → 停止 → "不存在" ✗ 错误！
```
...
class HashMap {
    ...
    struct Slot {
        ...
        bool deleted  = false;
        // ↑ 被惰性删除了吗？deleted==true 的槽对查找来说是"跳过"，
        //   但对插入来说是"可以复用"。
    };
    ...
    int   cnt = 0;
    // ↑ 有效元素数（used==true 且 deleted==false）。
    //   当 cnt / cap >= 0.5 时触发扩容。
    ...
    void rebuild(int newCap) {
        ...
        //   我们写了 ()，所以所有 Slot 的 used/deleted 都自动设为 false。
        //   如果忘了写 ()，used/deleted 会是随机值 → 灾难！
        ...
        //   因为是全新的表（cnt=0, slots 全是处女槽），不会有冲突累积。
...
public:
    ...
    V& operator[](int k) {
        ...
        int firstDel = -1;
        // ↑ firstDel：遇到的第一个 deleted 槽的下标。初始 -1 表示"还没遇到"。
        //   如果整个探测链上没有空位，我们可以复用这个 deleted 槽。
        ...
        while (slots[i].used) {
            ...
            if (slots[i].deleted) {
                ...
                // ↑ 记下第一个 deleted 槽的位置，但继续探测（key 可能在更后面）
        ...
        // ↑ 优先复用 deleted 槽，没有则用遇到的第一个处女槽。

        slots[dest] = { k, V{}, true, false };
        // ↑ 聚合初始化（Aggregate Initialization）：
        //   按 Slot 中成员的声明顺序依次赋值：
        //     key=k, value=V{}, used=true, deleted=false
        //
        //   V{} 是什么？
    ...
    V* find(int k) {
        ...
        // ↑ s 记住起始位置。如果绕了一圈回到起始，说明表满了或全是 deleted。
    ...
    const V* find(int k) const {
        ...
        unsigned i = hashIndex(k), s = i;
        while (slots[i].used) {
            if (!slots[i].deleted && slots[i].key == k) return &slots[i].value;
            if ((i = (i + 1) % (unsigned)cap) == s) break;
        }
        return nullptr;
    }
    ...
    void clear() {
        ...
        // ↑ 全部标回"处女槽"。不释放内存（delete[] slots 再 new），
...
// 测试 5：删除后重新插入
dist[2] = 9.9;
// ↑ 会优先复用之前惰性删除的 slot（key=2 的 deleted 槽）
